<template>
  <div>
    <b-card class="text-left">
      <b-card-text>
        <div> <a <router-link class="user" v-bind:to="{ path: `/user/${getObject.user.id}` }"> <button type="button"  class="btn btn-outline-danger open-button float-right" @click='reloadPage()'>Visitar</button> </router-link></a> </div>
        <h4><b>{{ getObject.user.nome }}</b>: {{ getObject.descricao }}</a></h4>
